...
public final class Storage<T> implements Closeable {
  ...
  private ReadableResult<T> doReadNext(SignalDeserializer<T> deserializer, int attemptNumber)
    ...
    if (readableFile == null) {
      ...
      readableFile = folderManager.getReadableFile(Objects.requireNonNull(fileExclusion.get()));
    ...
    fileExclusion.set(file -> file.createdTimeMillis <= currentFileCreatedTime);
    readableFile.close();
