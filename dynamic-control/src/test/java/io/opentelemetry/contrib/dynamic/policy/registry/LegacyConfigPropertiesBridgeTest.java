...
class LegacyConfigPropertiesBridgeTest {
  ...
  void exposesGeneralScalarPropertiesThroughTheDeclarativeBridge() {
    ...
    DeclarativeConfigProperties properties = componentProperties(config);

    assertThat(properties.getString("otel.opamp.service.url")).isEqualTo("https://example.com");
  }

  @Test
  void extractsLegacyMapsSeparatelyFromTheDeclarativeBridge() {
    ConfigProperties config = mock(ConfigProperties.class);
    ...
    assertThat(LegacyConfigPropertiesBridge.getResourceAttributes(config))
        .containsEntry("service.name", "legacy-service")
