 ...
 */
public class AdditionalClassesInjector implements HelperInjectorListener {

  @Override
  public void onInjection(Map<String, byte[]> classnameToBytes) {
    for (Map.Entry<String, byte[]> classEntry : classnameToBytes.entrySet()) {
      String modifiedClassName = classEntry.getKey().replace(".", "/") + ".class";
      AdditionalClasses.put(modifiedClassName, classEntry.getValue());
    }
  }
}
