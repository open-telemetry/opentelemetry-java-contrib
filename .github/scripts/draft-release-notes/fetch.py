#!/usr/bin/env python3
"""Build the local review bundle that the classifier consumes.

Walks the commit range for the pending release and writes one directory per
PR under build/changelog-bundle/prs/<N>/ containing the patch, metadata, body
and comments. Produces no stdout output; classify.py reads the bundle next.

Incremental by default: a PR whose recorded commit hash still matches is
reused. Pass --refetch to re-download everything.
"""
