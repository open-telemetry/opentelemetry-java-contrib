...
public class UserDefinedMessageSystemTest extends AbstractBaseTest {
  ...
  void setupClass() {
    ...
    MessagingProcessWrapper<MessageRequest> wrapper = MessagingProcessWrapper.<MessageRequest>defaultBuilder()
        .openTelemetry(otel)
  ...
  public void sendWithParent(Tracer tracer) {
    // mock a send span
    Span parent = tracer.spanBuilder("publish " + EVENTBUS_NAME)
        .setSpanKind(SpanKind.PRODUCER)
        .startSpan();

    try (Scope scope = parent.makeCurrent()) {
      Message message = Message.create(new HashMap<>(), MESSAGE_ID, MESSAGE_BODY);
      otel.getPropagators().getTextMapPropagator().inject(Context.current(), message, MessageTextMapSetter.create());
